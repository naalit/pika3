        ...
        if let Ok(elab) = db.elab.def_value(def, db) {
            ...
            for (m, source, ty) in elab.unsolved_metas {
                if root_cxt.meta_val(m).is_none() {
                    source_map.insert(m, source.clone());
                    root_cxt
                        .metas
...
pub fn elab_def(def: Def, db: &DB) -> Option<DefElabResult> {
    ...
    let mut def_elab = elab_def_raw(def, pre, &[], &mut cxt);
    let keys = cxt
        .child_defs
        .with(|x| x.keys().copied().collect::<Vec<_>>());
    def_elab.children.extend(
        keys.into_iter()
            .map(|k| (k, cxt.try_local_def_elab(k).unwrap())),
    );
    Some(DefElabResult {
        def: def_elab,
        unsolved_metas: cxt.metas.with(|m| {
            m.iter()
                .filter_map(|(m, v)| match v {
                    MetaEntry::Unsolved(t, source) => Some((*m, source.clone(), t.clone())),
...
fn elab_def_raw(def: Def, pre: &S<PreDef>, scope_vars: &[Sym], cxt: &mut Cxt) -> DefElab {
    let mspine: Arr<_> = scope_vars
        .iter()
    ...
    match &**pre {
        PreDef::Val { name, ty, value } => {
            ...
            let solved_ty = match &ty {
                Some(ty) => {
                    ...
                    cxt.solve_meta(Meta(def, 0), &mspine, ty, Some(name.span()));
                    true
                }
                None => false,
            };
            ...
            if !solved_ty {
                let ety = Val::Neutral(Head::Meta(Meta(def, 0)), mspine.to_vec()).zonk(cxt, true);
                if !ty
                    .clone()
                    .glued()
        ...
        PreDef::Type {
            ...
            let ty = m
                ...
                .eval(cxt.env())
                .zonk(cxt, true);
            cxt.solve_meta(Meta(def, 0), &mspine, ty.clone(), Some(name.span()));
            let base_rty = scope_vars
                .iter()
                .fold(Term::Head(Head::Def(def)), |acc, s| {
                    Term::App(
                        Box::new(acc),
            ...
            for (vname, vargs, vty) in variants {
                let vty = match vty {
                    ...
                    None => {
                        ...
                        let rty = m.iter().fold(base_rty.clone(), |acc, (i, (s, _))| {
                            Term::App(
                                Box::new(acc),
...
impl Pretty for MetaSource {
    fn pretty(&self, db: &DB) -> Doc {
        match self {
            MetaSource::TypeOf(n) => "inferred type of " + n.pretty(db),
            MetaSource::ImpArg(n) => "implicit argument " + n.pretty(db),
            MetaSource::Hole => "value of _".into(),
            MetaSource::Unknown => "metavariable".into(),
            MetaSource::Other(s) => s.into(),
        }
    }
}
...
impl Val {
    fn is_owned(&self) -> bool {
        match self {
            Val::Neutral(Head::Builtin(Builtin::Region | Builtin::UnitType), v) if v.is_empty() => {
                false
            }
    ...
    fn check(&self, cxt: &Cxt, span: Span) -> bool {
        ...
        for (s, b) in &self.borrows {
            if let Some(e) = cxt.vars.get(&s) {
                ...
                okay = false;
                // TODO this can happen when returning something from a function that borrows locals, that should be possible to do but we need to check somewhere that they haven't been invalidated
...
#[derive(Clone)]
enum ChildDefState {
    NotStarted(Vec<Sym>, Arc<S<PreDef>>, Cxt),
    InProgress,
    Finished(DefElab),
}
...
struct Cxt {
    db: DB,
    def: Def,
    bindings: im::HashMap<Name, VarBinding>,
    ...
    child_defs: Ref<FxHashMap<Def, ChildDefState>>,
...
impl Cxt {
    ...
    fn try_local_def_elab(&self, def: Def) -> Result<DefElab, DefElabError> {
        match self.child_defs.with(|x| x.get(&def).cloned()) {
            Some(ChildDefState::Finished(elab)) => Ok(elab),
            Some(ChildDefState::InProgress) => Err(DefElabError::Recursive),
            Some(ChildDefState::NotStarted(scope, pre, mut cxt)) => {
                self.child_defs
                    .with_mut(|x| x.insert(def, ChildDefState::InProgress));
                let elab = elab_def_raw(def, &pre, &scope, &mut cxt);
                let body = match &elab.body {
                    Some(DefBody::Val(body)) => Some(DefBody::Val(Arc::new(
                        scope.iter().rfold((**body).clone(), |acc, s| {
                            Term::fun(Lam, Expl, *s, None, Term::Error, Arc::new(acc))
                        }),
                    ))),
                    body => body.clone(),
                };
                let ty = scope.iter().rfold(elab.ty.clone(), |acc, s| {
                    Arc::new(Val::fun(
                        Pi(FCap::Imm),
                        Expl,
                        *s,
                        Arc::new(Val::Error),
                        None,
                        Arc::new(acc.quote(self.qenv())),
                        // TODO these envs are definitely not correct... not sure if it matters
                        Arc::new(self.env().clone()),
                    ))
                });

                let elab = DefElab { body, ty, ..elab };
                self.child_defs
    ...
    fn def_elab(&self, def: Def) -> Option<DefElab> {
        match self.try_local_def_elab(def) {
            Ok(elab) => Some(elab),
            Err(DefElabError::Recursive) => None,
            Err(DefElabError::NotFound) => {
                self.db.elab.def_value(def, &self.db).ok().map(|x| x.def)
            }
        }
    }

    fn def_value(&self, def: Def) -> Option<Val> {
        let elab = self.def_elab(def)?;
        elab.can_eval
            .then(|| {
                elab.body.and_then(|x| match x {
                    DefBody::Val(x) => Some(x.eval(self.env())),
                    DefBody::Type(_) => None,
                })
            })
            .flatten()
    ...
    fn def_type(&self, def: Def) -> Result<Arc<Val>, DefElabError> {
        match self.try_local_def_elab(def) {
            Ok(elab) => Ok(elab.ty),
            Err(DefElabError::Recursive) => Err(DefElabError::Recursive),
            Err(DefElabError::NotFound) => self.db.elab.def_type(def, &self.db),
        }
    }

    fn pre_bind_local_def(&mut self, pre: &S<PreDef>) -> Def {
        let name = pre.name();
        let def = self.db.idefs.intern(&DefLoc::Child(
            self.def,
            // avoid conflicts
            self.db.name(&format!(
                "{}${}",
                self.db.get(*name),
                self.child_defs.with(|x| x.len())
            )),
        ));
        let syms: Vec<_> = self
            .vars
            ...
            .collect();
        let scope: Vec<_> = syms
            .iter()
            .map(|&s| VElim::App(Expl, Arc::new(Val::sym(s))))
            .collect();
        self.bindings
            .insert(*name, VarBinding::LocalDef(def, scope));
        def
    }

    /// Assumes you've called `pre_bind_local_def` (to get the `Def` to pass to this method)
    fn bind_local_def(&mut self, def: Def, pre: S<PreDef>) {
        let syms: Vec<_> = self
            .vars
            .keys()
            .copied()
            ...
            .collect();
        self.child_defs.with_mut(|v| {
            v.insert(
                def,
                ChildDefState::NotStarted(syms, Arc::new(pre), self.clone()),
            )
        });
    }
    ...
    fn lookup(&self, n: SName) -> Option<VarResult> {
        ...
        self.bindings
            .get(&n.0)
            .and_then(|s| match s {
                VarBinding::Local(s) => self
                    .vars
                    .get(s)
                    .map(|entry| VarResult::Local(n.span(), entry)),
                VarBinding::LocalDef(def, spine) => {
                    let ty = self.def_type(*def).unwrap_or_else(|e| match e {
                        DefElabError::NotFound => unreachable!(),
                        DefElabError::Recursive => {
                            Arc::new(Val::Neutral(Head::Meta(Meta(*def, 0)), spine.clone()))
                        }
                    });
                    let ty = Arc::new(
                        spine
                            .iter()
                            .fold((*ty).clone(), |acc, x| x.clone().elim(acc)),
                    );
                    let term = spine.iter().fold(Term::Head(Head::Def(*def)), |acc, x| {
                        Term::App(Box::new(acc), x.quote_(self.qenv()).unwrap())
                    });
                    Some(VarResult::Other(term, ty))
                }
            })
            .or_else(|| {
    ...
    fn new_meta(&self, ty: Val, source: MetaSource, span: Span) -> Val {
        ...
        {
            let m1 = self.new_meta((**aty).clone(), source.clone(), span);
    ...
    fn meta_source(&self, m: Meta) -> Option<S<MetaSource>> {
        self.metas.with(|x| {
            x.get(&m).and_then(|x| match x {
                MetaEntry::Unsolved(_, source) => Some(source.clone()),
                MetaEntry::Solved(_) => None,
            })
        })
    }
    ...
    fn solve_meta(&self, meta: Meta, spine: &[VElim], solution: Val, span: Option<Span>) -> bool {
        ...
        if spine.iter().any(|x| !matches!(x, VElim::App(_, _))) {
            self.err(
                "cannot solve meta with non-app in spine: "
                    + Val::Neutral(Head::Meta(meta), spine.to_vec()).pretty(&self.db),
                span,
            );
        }
...
fn elab_block(block: &[PreStmt], last_: &SPre, ty: Option<GVal>, cxt1: &Cxt) -> (Term, Val) {
    ...
    let mut v = Vec::new();
    let mut def_acc = Vec::new();
    for x in block {
        if !matches!(x, PreStmt::Def(_)) {
            def_acc
                .drain(..)
                .for_each(|(x, y)| cxt.bind_local_def(x, y));
        }
        let (vsym, deps, t, p, x, span) = match x {
            ...
            PreStmt::Def(def) => {
                let def_id = cxt.pre_bind_local_def(def);
                def_acc.push((def_id, def.clone()));
                continue;
            }
        };
        let deps = t
    ...
    fn infer_(&self, cxt: &Cxt, mut should_insert_metas: bool, borrow_as: Cap) -> (Term, GVal) {
        ...
        let (s, sty) = match &***self {
            Pre::Hole(source) => {
                // hole
                let mty =
                    cxt.new_meta(Val::Type, MetaSource::TypeOf(cxt.db.name("_")), self.span());
                let m = cxt
                    .new_meta(
                        mty.clone(),
                        match source {
                            // TODO can we do something with this span?
                            crate::parser::HoleSource::TypeOf(s) => MetaSource::TypeOf(**s),
                        },
                        self.span(),
                    )
                    .quote(&cxt.qenv());
            ...
            Pre::Case(x, branches) => {
                let rty = cxt.new_meta(
                    Val::Type,
                    MetaSource::Other("return type of `case`".into()),
                    self.span(),
                );
