...
pub fn all_errors(file: File, db: &DB) -> ModuleElabResult {
    ...
    for i in r.defs {
        ...
        let (body, ty) = if let Some((val, ty)) = i.0.value.as_ref().map(|val| match &ty {
            Some(ty) => {
...
impl Val {
    fn quote(&self, env: &QEnv) -> Term {
        match self {
            Val::Neutral(s, spine) => spine.iter().fold(env.get(*s), |acc, x| {
